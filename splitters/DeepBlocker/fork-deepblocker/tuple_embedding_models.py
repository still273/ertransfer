...
class AverageEmbedding(ABCTupleEmbedding):
    def __init__(self, embedding_path='embedding/'):
        super().__init__()
        print("Loading FastText model")

        self.word_embedding_model = fasttext.load_model(embedding_path + "/" + FASTTEXT_EMBEDDIG_PATH)
...
class SIFEmbedding(ABCTupleEmbedding):
    ...
    def __init__(self, sif_weighting_param=1e-3, remove_pc=True, min_freq=0, embedding_path='embedding/'):
        super().__init__()
        print("Loading FastText model")
...
class AutoEncoderTupleEmbedding(ABCTupleEmbedding):
    def __init__(self, hidden_dimensions=(2*AE_EMB_DIMENSION_SIZE, AE_EMB_DIMENSION_SIZE), embedding_path='embedding/'):
        super().__init__()
        self.input_dimension = EMB_DIMENSION_SIZE
        self.hidden_dimensions = hidden_dimensions
        self.sif_embedding_model = SIFEmbedding(embedding_path=embedding_path)
...
class CTTTupleEmbedding(ABCTupleEmbedding):
    def __init__(self, hidden_dimensions=(2*AE_EMB_DIMENSION_SIZE, AE_EMB_DIMENSION_SIZE),
            synth_tuples_per_tuple=5, pos_to_neg_ratio=1, max_perturbation=0.4, embedding_path='embedding/'):
        super().__init__()
        self.input_dimension = EMB_DIMENSION_SIZE
        self.hidden_dimensions = hidden_dimensions
