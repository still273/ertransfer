...
def join_columns(table, columns_to_join=None, separator=' '):
    if columns_to_join == None:
        columns = [column for column in table.columns if column != 'id']
    else:
    ...
    join_table = red_table.aggregate(separator.join, axis=1)
...
def transform_input(source_dir, columns_to_join=None, separator=' ', prefixes=['tableA_', 'tableB_']):
    ...
    train = train_df[['tableA_id', 'tableB_id', 'label']]

    tableA = pd.read_csv(os.path.join(source_dir, 'tableA.csv'), encoding_errors='replace')
    tableB = pd.read_csv(os.path.join(source_dir, 'tableB.csv'), encoding_errors='replace')
    tableA = join_columns(tableA, columns_to_join, separator)
    tableB = join_columns(tableB, columns_to_join, separator)

    return train, tableA, tableB
